...
export default function PropertyCard({ property, isFavorite, toggleFavorite }) {
  return (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden w-64 hover:shadow-xl hover:shadow-mint/60 transition cursor-pointer featured-card">
      ...
      <div className="p-4 text-left">
        ...
        <div className="flex justify-between items-center mt-3">
          <span className="text-us-primary font-semibold">
            ${property.price} <span className="text-gray-500 text-sm">/mo</span>
          </span>
        ...
        <div className="flex items-center gap-3 mt-3 text-gray-500 text-sm">
          {property.features.map((f, idx) => (
            <span key={idx}>
              <i
                className={`fa-solid fa-${f} text-us-textDark/60`}
              ></i>{" "}
              <span className="text-xs text-us-textDark/60">
